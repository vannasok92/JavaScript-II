...
let firstNamesAllCaps = runners.map(element => element.first_name.toUpperCase());
...
let ticketPriceTotal = runners.reduce((acc,element) => acc + element.donation,0);
console.log(`Total donation is: ${ticketPriceTotal}`);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.

// Problem 1: list all runners first name is alphapetical order 
let newName = [];
runners.forEach(element=>{
  let fn = element.first_name;
  newName.push(fn);
});
console.log(newName.sort());
// Problem 2
